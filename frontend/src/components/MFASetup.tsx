...
const MFASetup: React.FC = () => {
  ...
  const addMfaMethod = async () => {
    ...
    try {
      ...
      console.log(
        `Verification code sent to ${newMethodValue} via ${newMethodType}`
      );
    } catch (err) {
            ...
            <>
              <div>
                ...
                <select
                  ...
                  onChange={e =>
                    setNewMethodType(
                      e.target.value as 'email' | 'sms' | 'authenticator'
                    )
                  }
        ...
        <div className='space-y-2 text-sm'>
          ...
          <div className='flex justify-between'>
            ...
            <span className='text-gray-800 dark:text-white'>
              {verifiedMethods.length} method
              {verifiedMethods.length !== 1 ? 's' : ''}
            </span>
          </div>
        </div>
      </div>
